...
public class ForumTypeController extends BaseController<ForumType, ForumTypeService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        // 将请求体转换为对象
        ForumType forum_type = convertRequestBodyToObject(request);
        // 验证参数是否合法
        validateObject(forum_type);
        this.addEntity(forum_type);
        System.out.println("论坛分类新增成功");
        return success(1);
    }
    ...
    public Map<String, Object> set(HttpServletRequest request) throws IOException {
        ForumType forum_type = convertRequestBodyToObject(request);
        this.service.updateEntity(service.readQuery(request), service.readConfig(request), forum_type);
        return success(1);
    }
     ...
     * 将请求体转换为对象
     */
    private ForumType convertRequestBodyToObject(HttpServletRequest request) throws IOException {
        Map<String, Object> bodyMap = service.readBody(request.getReader());
        return JSON.parseObject(JSON.toJSONString(bodyMap), ForumType.class);
    }

    /**
     * 验证对象的参数是否合法
     */
    private void validateObject(ForumType forum_type) {
            if (isNullOrBlank(forum_type.getName())) {
