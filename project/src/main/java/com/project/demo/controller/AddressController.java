...
public class AddressController extends BaseController<Address, AddressService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        // 将请求体转换为对象
        Address address = convertRequestBodyToObject(request);
        // 验证参数是否合法
    ...
    public Map<String, Object> set(HttpServletRequest request) throws IOException {
        Address address = convertRequestBodyToObject(request);
        this.service.updateEntity(service.readQuery(request), service.readConfig(request), address);
        return success(1);
    }
     ...
     * 将请求体转换为对象
     */
    private Address convertRequestBodyToObject(HttpServletRequest request) throws IOException {
        Map<String, Object> bodyMap = service.readBody(request.getReader());
        return JSON.parseObject(JSON.toJSONString(bodyMap), Address.class);
    }

    /**
     * 验证对象的参数是否合法
     */
    private void validateObject(Address address) {
            if (isNullOrBlank(address.getAddress())) {
