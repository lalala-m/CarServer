...
public class ForumController extends BaseController<Forum, ForumService> {
    ...
    public Map<String, Object> add(HttpServletRequest request) throws IOException {
        // 将请求体转换为对象
        Forum forum = convertRequestBodyToObject(request);
        // 验证参数是否合法
        validateObject(forum);
        this.addEntity(forum);
        System.out.println("论坛数据新增成功");
        return success(1);
    }
    ...
    public Map<String, Object> set(HttpServletRequest request) throws IOException {
        Forum forum = convertRequestBodyToObject(request);
        this.service.updateEntity(service.readQuery(request), service.readConfig(request), forum);
        return success(1);
    }
     ...
     * 将请求体转换为对象
     */
    private Forum convertRequestBodyToObject(HttpServletRequest request) throws IOException {
        Map<String, Object> bodyMap = service.readBody(request.getReader());
        return JSON.parseObject(JSON.toJSONString(bodyMap), Forum.class);
    }

    /**
     * 验证对象的参数是否合法
     */
    private void validateObject(Forum forum) {
            if (isNullOrBlank(forum.getTitle())) {
