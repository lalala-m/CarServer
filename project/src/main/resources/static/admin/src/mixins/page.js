export default {
    ...
    methods: {
        ...
        get_list: function get_list(param, func) {
            ...
            for(let key in param){
                if (Array.isArray(param[key])){
                    qy[key + '_min'] = new Date(param[key][0]).toStr('yyyy-MM-dd hh:mm:ss');
                    qy[key + '_max'] = new Date(param[key][1]).toStr('yyyy-MM-dd hh:mm:ss');
                }else{
                    qy[key] = param[key]
                }
            }
            var pm = this.events("get_list_before", Object.assign({}, qy)) || qy;
        ...
        get_list_main: function get_list_main(query, func) {
            ...
            // 页面重载参数添加
            this.$router.replace({query: {...this.$route.query,...$.delete(this.query)},})
            .catch((err) => {
            console.warn(err);
            });
        ...
        reset: async function reset() {
            await this.$router.push({ query: {} }).catch(err => {});
            $.clear(this.query);
