<template>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
																		<el-button v-if="$check_option('/write_off_information/table','print')" @click="print('dataTable')" style="margin-right: 155px;" class="search_btn_print">打印</el-button>
														

														<el-button v-if="$check_action('/write_off_information/table','del') || $check_action('/write_off_information/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="write_off_information_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="card_code" @sort-change="$sortChange" label="卡券编码" 				v-if="$check_field('get','card_code')" min-width="300">
					...
					<el-table-column prop="write_off_time" @sort-change="$sortChange" label="核销时间" 				v-if="$check_field('get','write_off_time')" min-width="300">
						<template slot-scope="scope">
									                	{{ $toTime(scope.row["write_off_time"],"yyyy-MM-dd hh:mm:ss") }}
																		</template>
					</el-table-column>
					<el-table-column prop="write_off_doc" @sort-change="$sortChange" label="核销单" 				v-if="$check_field('get','write_off_doc')" min-width="300">
						<template slot-scope="scope">
					<el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['write_off_doc'])"
						:preview-src-list="[$fullUrl(scope.row['write_off_doc'])]">
					...
					<el-table-column prop="write_off_remarks" @sort-change="$sortChange" label="核销备注" 				v-if="$check_field('get','write_off_remarks')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["write_off_remarks"] }}
									</template>
					</el-table-column>
			...
			<el-table-column label="操作" min-width="200" v-if="$check_action('/write_off_information/table','set') || $check_action('/write_off_information/view','set') || $check_action('/write_off_information/view','get')
						" >
