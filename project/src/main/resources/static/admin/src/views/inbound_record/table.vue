<template>
			...
			<el-row class="rows row1">
					...
					<el-form-item label="耗材名称">
									<el-input v-model="query.consumables_name"></el-input>
															...
															<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap" v-if="$check_field('get','inbound_time')">
					<el-form-item label="入库时间">
									<el-date-picker v-model="query.inbound_time" type="datetimerange" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间"></el-date-picker>
								</el-form-item>
				</el-col>
										</el-row>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
																		<el-button v-if="$check_option('/inbound_record/table','print')" @click="print('dataTable')" style="margin-right: 155px;" class="search_btn_print">打印</el-button>
														

														<el-button v-if="$check_action('/inbound_record/table','del') || $check_action('/inbound_record/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="inbound_record_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="consumables_code" @sort-change="$sortChange" label="耗材编码" 				v-if="$check_field('get','consumables_code')" min-width="300">
					...
					<el-table-column prop="inbound_time" @sort-change="$sortChange" label="入库时间" 				v-if="$check_field('get','inbound_time')" min-width="300">
						<template slot-scope="scope">
									                	{{ $toTime(scope.row["inbound_time"],"yyyy-MM-dd hh:mm:ss") }}
																		</template>
					</el-table-column>
					<el-table-column prop="inbound_quantity" @sort-change="$sortChange" label="入库数量" 				v-if="$check_field('get','inbound_quantity')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["inbound_quantity"] }}
									</template>
					</el-table-column>
					<el-table-column prop="inbound_notes" @sort-change="$sortChange" label="入库备注" 				v-if="$check_field('get','inbound_notes')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["inbound_notes"] }}
									</template>
					</el-table-column>
			...
			<el-table-column label="操作" min-width="200" v-if="$check_action('/inbound_record/table','set') || $check_action('/inbound_record/view','set') || $check_action('/inbound_record/view','get')
						" >
