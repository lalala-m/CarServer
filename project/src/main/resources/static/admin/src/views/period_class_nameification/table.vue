<template>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
														...
														<el-button v-if="$check_action('/period_class_nameification/table','del') || $check_action('/period_class_nameification/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="period_class_nameification_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
					<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
			</el-table-column>
				<el-table-column prop="time_period" @sort-change="$sortChange" label="时间时段" 				v-if="$check_field('get','time_period')" min-width="300">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/period_class_nameification/table','set') || $check_action('/period_class_nameification/view','set') || $check_action('/period_class_nameification/view','get')
						" >
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/period_class_nameification/get_list",
		...
		methods: {
			...
			get_list_after: function get_list_after(res, func, url) {
				...
				if (res.result && res.result.list && res.result.list.length > 0) {
					this.$message({
						type: 'error',
						message: '"' + type.time_period + '"下有关联的内容，无法删除！'
					});
					return;
				}
																																																																					}
