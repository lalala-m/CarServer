	...
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
							...
							<el-form-item label="耗材编码" prop="consumables_code">
							  ...
							  v-if="(form['inbound_record_id'] && $check_field('set','consumables_code')) || (!form['inbound_record_id'] && $check_field('add','consumables_code'))" :disabled="disabledObj['consumables_code_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','consumables_code')">
						{{form['consumables_code']}}
							...
							<el-form-item label="耗材名称" prop="consumables_name">
							  ...
							  v-if="(form['inbound_record_id'] && $check_field('set','consumables_name')) || (!form['inbound_record_id'] && $check_field('add','consumables_name'))" :disabled="disabledObj['consumables_name_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','consumables_name')">
						{{form['consumables_name']}}
							...
							<el-form-item label="耗材规格" prop="consumables_specifications">
							  ...
							  v-if="(form['inbound_record_id'] && $check_field('set','consumables_specifications')) || (!form['inbound_record_id'] && $check_field('add','consumables_specifications'))" :disabled="disabledObj['consumables_specifications_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','consumables_specifications')">
						{{form['consumables_specifications']}}
							...
							<el-select v-if="(form['inbound_record_id'] && $check_field('set','financial_user')) || (!form['inbound_record_id'] && $check_field('add','financial_user'))" id="financial_user" v-model="form['financial_user']" :disabled="disabledObj['financial_user_isDisabled']">
						...
						<el-select v-else id="financial_user" v-model="form['financial_user']" :disabled="disabledObj['financial_user_isDisabled']">
							<el-option v-for="o in list_user_financial_user" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
																...
																<el-col v-if="$check_field('get','inbound_time') || $check_field('add','inbound_time') || $check_field('set','inbound_time')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
							<el-form-item label="入库时间" prop="inbound_time">
											<el-date-picker :disabled="disabledObj['inbound_time_isDisabled']" v-if="(form['inbound_record_id'] && $check_field('set','inbound_time')) || (!form['inbound_record_id'] && $check_field('add','inbound_time'))" id="inbound_time"
						v-model="form['inbound_time']" type="datetime" placeholder="选择日期时间">
					</el-date-picker>
					<div v-else-if="$check_field('get','inbound_time')">{{form['inbound_time']}}</div>
										</el-form-item>
			</el-col>
																<el-col v-if="$check_field('get','inbound_quantity') || $check_field('add','inbound_quantity') || $check_field('set','inbound_quantity')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
							<el-form-item label="入库数量" prop="inbound_quantity">
											<el-input-number id="inbound_quantity" v-model.number="form['inbound_quantity']"
						v-if="(form['inbound_record_id'] && $check_field('set','inbound_quantity')) || (!form['inbound_record_id'] && $check_field('add','inbound_quantity'))" :disabled="disabledObj['inbound_quantity_isDisabled']"></el-input-number>
					<span v-else-if="$check_field('get','inbound_quantity')">{{form['inbound_quantity']}}</span>
														</el-form-item>
			</el-col>
																<el-col v-if="$check_field('get','inbound_notes') || $check_field('add','inbound_notes') || $check_field('set','inbound_notes')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
							<el-form-item label="入库备注" prop="inbound_notes">
											<el-input type="textarea" id="inbound_notes" v-model="form['inbound_notes']" placeholder="请输入入库备注"
						v-if="(form['inbound_record_id'] && $check_field('set','inbound_notes')) || (!form['inbound_record_id'] && $check_field('add','inbound_notes'))" :disabled="disabledObj['inbound_notes_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','inbound_notes')">{{form['inbound_notes']}}</div>
										</el-form-item>
			</el-col>
				...
				<el-form-item v-if="$check_action('/inbound_record/view','set') || $check_action('/inbound_record/view','add')">
					<el-button type="primary" @click="submit()">提交</el-button>
					<el-button @click="cancel()">取消</el-button>
					<el-button v-if="!!query.inbound_record_id"  type="primary" @click="sendprint">打印</el-button>
				</el-form-item>
				<el-form-item v-else>
	...
	import mixin from "@/mixins/page.js";
		...
		data() {
			return {
				...
				url_get_list: "~/api/inbound_record/get_list?",
				...
				disabledObj:{
								"consumables_code_isDisabled": false,
										...
										"inbound_time_isDisabled": false,
					          			"inbound_quantity_isDisabled": false,
		...
		methods: {
				...
				let formArrs = [
									{ label: '耗材编码', key: 'consumables_code', value: '' },
										...
										{ label: '入库时间', key: 'inbound_time', value: '' },
										{ label: '入库数量', key: 'inbound_quantity', value: '' },
										{ label: '入库备注', key: 'inbound_notes', value: '' },
										]
			...
			async get_obj_after(json, func){
				if (json.result && json.result.obj) {
					const obj = json.result.obj;
																																					...
																																					if(this.form["inbound_time"]=="0000-00-00 00:00:00"){
				  this.form["inbound_time"] = null;
				}
				if(parseInt(this.form["inbound_time"]) > 9999){
					this.form["inbound_time"] = this.$toTime(parseInt(this.form["inbound_time"]),"yyyy-MM-dd hh:mm:ss")
				}
				...
				return ret;
					...
					async submit_after(json, func) {
						...
						let skip_Path  = path.replace('/view','/table')
								...
								title: '入库记录',
								type: '消息',
								content: this.$store.state.user.username + '-' + this.$store.state.user.nickname + '提交了一条入库记录数据',
								state: 1,
								user_id:'9999'
							}
