<template>
			...
			<el-row class="rows row1">
					...
					<el-form-item label="耗材名称">
									<el-input v-model="query.consumables_name"></el-input>
															...
															<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap" v-if="$check_field('get','outbound_time')">
					<el-form-item label="出库时间">
									<el-date-picker v-model="query.outbound_time" type="datetimerange" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间"></el-date-picker>
								</el-form-item>
				</el-col>
										</el-row>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
																		<el-button v-if="$check_option('/outbound_record/table','print')" @click="print('dataTable')" style="margin-right: 155px;" class="search_btn_print">打印</el-button>
														

														<el-button v-if="$check_action('/outbound_record/table','del') || $check_action('/outbound_record/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="outbound_record_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="consumables_code" @sort-change="$sortChange" label="耗材编码" 				v-if="$check_field('get','consumables_code')" min-width="300">
					...
					<el-table-column prop="outbound_time" @sort-change="$sortChange" label="出库时间" 				v-if="$check_field('get','outbound_time')" min-width="300">
						<template slot-scope="scope">
									                	{{ $toTime(scope.row["outbound_time"],"yyyy-MM-dd hh:mm:ss") }}
																		</template>
					</el-table-column>
					<el-table-column prop="outbound_quantity" @sort-change="$sortChange" label="出库数量" 				v-if="$check_field('get','outbound_quantity')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["outbound_quantity"] }}
									</template>
					</el-table-column>
					<el-table-column prop="outbound_note" @sort-change="$sortChange" label="出库备注" 				v-if="$check_field('get','outbound_note')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["outbound_note"] }}
									</template>
					</el-table-column>
			...
			<el-table-column label="操作" min-width="200" v-if="$check_action('/outbound_record/table','set') || $check_action('/outbound_record/view','set') || $check_action('/outbound_record/view','get')
						" >
