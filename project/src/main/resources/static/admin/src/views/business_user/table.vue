<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="$check_action('/business_user/table','add') || $check_action('/business_user/view','add')" @click="$router.push('./view?')">添加</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="business_user_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			...
			<el-table-column prop="username" label="用户名" width="120">
				...
				<el-table-column fixed prop="nickname" label="昵称">
					...
					<el-table-column prop="business_name" @sort-change="$sortChange" label="商家姓名" 				v-if="$check_field('get','business_name')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["business_name"] }}
									</template>
					</el-table-column>
					<el-table-column prop="business_gender" @sort-change="$sortChange" label="商家性别" 				v-if="$check_field('get','business_gender')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["business_gender"] }}
									</template>
					</el-table-column>
					<el-table-column prop="mobile_phone_number" @sort-change="$sortChange" label="手机号码" 				v-if="$check_field('get','mobile_phone_number')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["mobile_phone_number"] }}
									</template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/business_user/table','set') || $check_action('/business_user/view','set') || $check_action('/business_user/view','get')
						" >
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/business_user/get_list",
