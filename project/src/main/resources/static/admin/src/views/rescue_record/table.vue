<template>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
														...
														<el-button v-if="$check_action('/rescue_record/table','del') || $check_action('/rescue_record/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="rescue_record_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
					<el-table-column fixed type="selection" tooltip-effect="dark" width="55">
			</el-table-column>
				<el-table-column prop="rescue_code" @sort-change="$sortChange" label="救援编码" 				v-if="$check_field('get','rescue_code')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["rescue_code"] }}
									</template>
					</el-table-column>
					...
					<el-table-column prop="rescue_user" @sort-change="$sortChange" label="救援用户" 				v-if="$check_field('get','rescue_user')" min-width="300">
						<template slot-scope="scope">
					{{ get_user_rescue_user(scope.row['rescue_user']) }}
				</template>
					</el-table-column>
					...
					<el-table-column prop="audit_status" @sort-change="$sortChange" label="审核状态" 				v-if="$check_field('get','audit_status')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["audit_status"] }}
									</template>
					</el-table-column>
					<el-table-column prop="maintenance_personnel" @sort-change="$sortChange" label="维修人员" 				v-if="$check_field('get','maintenance_personnel')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["maintenance_personnel"] }}
									</template>
					</el-table-column>
					<el-table-column prop="maintenance_time" @sort-change="$sortChange" label="维修时间" 				v-if="$check_field('get','maintenance_time')" min-width="300">
						<template slot-scope="scope">
									                	{{ $toTime(scope.row["maintenance_time"],"yyyy-MM-dd hh:mm:ss") }}
																		</template>
					</el-table-column>
					<el-table-column prop="scene_picture" @sort-change="$sortChange" label="现场图片" 				v-if="$check_field('get','scene_picture')" min-width="300">
						<template slot-scope="scope">
					<el-image style="width: 100px; height: 100px" :src="$fullUrl(scope.row['scene_picture'])"
						:preview-src-list="[$fullUrl(scope.row['scene_picture'])]">
					...
					<el-table-column prop="maintenance" @sort-change="$sortChange" label="维修情况" 				v-if="$check_field('get','maintenance')" min-width="300">
					<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/rescue_record/table','set') || $check_action('/rescue_record/view','set') || $check_action('/rescue_record/view','get')
						" >
