<template>
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
														...
														<el-button v-if="$check_action('/card_type/table','del') || $check_action('/card_type/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="card_type_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="card_name" @sort-change="$sortChange" label="卡券名称" 				v-if="$check_field('get','card_name')" min-width="300">
					...
					<el-table-column prop="effective_period" @sort-change="$sortChange" label="有效期限" 				v-if="$check_field('get','effective_period')" min-width="300">
		                <template slot-scope="scope">
									                	{{ $toTime(scope.row["effective_period"],"yyyy-MM-dd") }}
														                </template>
					</el-table-column>
					<el-table-column prop="card_content" @sort-change="$sortChange" label="卡券内容" 				v-if="$check_field('get','card_content')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["card_content"] }}
									</template>
					</el-table-column>
					<el-table-column prop="applicable_items" @sort-change="$sortChange" label="适用项目" 				v-if="$check_field('get','applicable_items')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["applicable_items"] }}
									</template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/card_type/table','set') || $check_action('/card_type/view','set') || $check_action('/card_type/view','get')
						" >
		...
		methods: {
			...
			get_list_after: function get_list_after(res, func, url) {
				...
				if (res.result && res.result.list && res.result.list.length > 0) {
					this.$message({
						type: 'error',
						message: '"' + type.card_name + '"下有关联的内容，无法删除！'
					});
					return;
				}
																																																																																																																																																																																																																																																																																																								}
