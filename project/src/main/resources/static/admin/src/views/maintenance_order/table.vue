<template>
			...
			<el-row class="rows row1">
					...
					<el-form-item label="项目套餐">
									<el-input v-model="query.project_bundle"></el-input>
																														...
																														<el-col :xs="24" :sm="24" :lg="8" class="el_form_search_wrap" v-if="$check_field('get','order_status')">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button @click="reset()" style="margin-right: 74px;" class="search_btn_reset">重置</el-button>
										...
										<el-button v-if="$check_option('/maintenance_order/table', 'examine')" @click="batchInfo()" class="examine_btn_state">批量审核</el-button>
														<el-button v-if="$check_action('/maintenance_order/table','del') || $check_action('/maintenance_order/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="maintenance_order_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			:default-expand-all="isExpandAll" ref="tableRef">
				...
				<el-table-column prop="project_bundle" @sort-change="$sortChange" label="项目套餐" 				v-if="$check_field('get','project_bundle')" min-width="300">
					...
					<el-table-column prop="order_status" @sort-change="$sortChange" label="订单状态" 				v-if="$check_field('get','order_status')" min-width="300">
					<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/maintenance_order/table','set') || $check_action('/maintenance_order/view','set') || $check_action('/maintenance_order/view','get')
						" >
				...
				<template slot-scope="scope">
					...
					v-if="$check_action('/maintenance_order/table','set') || $check_action('/maintenance_order/view','set') || $check_action('/maintenance_order/view','get')"
						...
						<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="openPayModal(scope.row)" v-if="scope.row.pay_state==='未支付' && ($check_pay('/maintenance_order/table'))  && scope.row['examine_state'] != '未通过'">
						<span>支付</span>
					</el-button>
						<el-button class="e-button el-button--small is-plain el-button--primary" style="margin: 5px !important;" size="small" @click="changeSigning(scope.row, scope.$index)" v-if="$check_option('/maintenance_order/table', 'examine') && (scope.row['examine_state'] == '未审核' || scope.row['examine_state'] == '审核中') ">
						<span>审核</span>
					</el-button>
				</div>
				</template>
	...
	export default {
		...
		methods: {
					...
					if (valid) {
						...
						if (this.batchAllState) {
							...
							for (var i = 0; i < this.batchAllList.length; i++) {
								let url = "~/api/maintenance_order/set?maintenance_order_id=" +  this.batchAllList[i]["maintenance_order_id"];
									...
									title: '审核结果',
									type: '通知',
									content: '你在保养订单下提交的内容，审核结果为：' + _this.verifyItem.examine_state,
									state: 1,
							...
							var url = "~/api/maintenance_order/set?maintenance_order_id=" + this.verifyItem['maintenance_order_id']
			...
			submitPay(){
				...
				let url = this.$toUrl(this.query, "~/api/maintenance_order/set?maintenance_order_id="+this.pay_obj.id);
