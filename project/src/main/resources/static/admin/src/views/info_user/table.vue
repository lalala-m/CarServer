<template>
	<el-main class="bg table_wrap comtabel_t">
			...
			<el-row class="rows row2">
				<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
					<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="$check_action('/info_user/table','add') || $check_action('/info_user/view','add')" @click="$router.push('./view?')">添加</el-button>
						<el-button v-if="$check_action('/info_user/table','del') || $check_action('/info_user/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
						<download-excel v-if="$check_option('/info_user/table','import_db')" class="export-excel-wrapper" :data="DetailsForm" :fields="json_fields" name="信息员数据导入表格.xls">
							<el-button type="success">下载导入文档</el-button>
						</download-excel>
						<el-upload v-if="$check_option('/info_user/table','import_db')" action accept=".xlsx, .xls" :auto-upload="false" :show-file-list="false" :on-change="handle_import">
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="info_user_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }" :default-expand-all="isExpandAll" ref="tableRef">
			...
			<el-table-column prop="employee_gender" @sort-change="$sortChange" label="员工性别" min-width="120">
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/info_user/table','set') || $check_action('/info_user/view','set') || $check_action('/info_user/view','get')">
				<template slot-scope="scope">
					<div class="view_a">
					<router-link class="e-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="$check_action('/info_user/table','set') || $check_action('/info_user/view','set') || $check_action('/info_user/view','get')"
						:to="'./view?info_user_id=' + scope.row['info_user_id']" size="small">
						<span>详情</span>
					</router-link>
					</div>
				</template>
						...
						if (json.result) {
							_this.$get("~/api/user/get_obj?", form_user, (res) => {
								if (res.result && res.result.obj) {
									...
									_this.$post("~/api/info_user/add?", result[i], function (json, status) {
										if (json.result) {
