<template>
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
												<el-col v-if="$check_field('get','time_period') || $check_field('add','time_period') || $check_field('set','time_period')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
							<el-form-item prop="time_period">
					<div slot="label">
						<i style="color: red;">*</i>
						时间时段
					</div>
															<el-input id="time_period" v-model="form['time_period']" placeholder="请输入时间时段"
							  v-if="(form['period_class_nameification_id'] && $check_field('set','time_period')) || (!form['period_class_nameification_id'] && $check_field('add','time_period'))" :disabled="disabledObj['time_period_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','time_period')">
						{{form['time_period']}}
				  					</div>
														</el-form-item>
			</el-col>
				...
				<el-form-item v-if="$check_action('/period_class_nameification/view','set') || $check_action('/period_class_nameification/view','add')">
				...
				return ret;
					...
					async submit_after(json, func) {
						...
						let skip_Path  = path.replace('/view','/table')
								...
								title: '时段分类',
								type: '消息',
								content: this.$store.state.user.username + '-' + this.$store.state.user.nickname + '提交了一条时段分类数据',
								state: 1,
								user_id:'9999'
							}
