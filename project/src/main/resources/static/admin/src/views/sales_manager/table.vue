<template>
	...
	<el-row class="rows row2">
		...
		<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap search_btns">
				...
				<el-col :xs="24" :sm="10" :lg="8" class="search_btn_1 search_btn_wrap_1 btns">
						...
						<el-button v-if="$check_action('/sales_manager/table','add') || $check_action('/sales_manager/view','add')" @click="$router.push('./view?')">添加</el-button>

						

														<el-button v-if="$check_action('/sales_manager/table','del') || $check_action('/sales_manager/view','del')" class="search_btn_del" type="danger" @click="delInfo()">删除</el-button>
	...
	<div class="statistics-info" v-if="statisticsData.total > 0">
				...
				<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable" row-key="sales_manager_id" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }"
			...
			<el-table-column prop="username" label="用户名" width="120">
				...
				<el-table-column fixed prop="nickname" label="昵称">
					...
					<el-table-column prop="manager_name" @sort-change="$sortChange" label="经理姓名" 				v-if="$check_field('get','manager_name')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["manager_name"] }}
									</template>
					</el-table-column>
					<el-table-column prop="gender_of_manager" @sort-change="$sortChange" label="经理性别" 				v-if="$check_field('get','gender_of_manager')" min-width="300">
					<template slot-scope="scope">
							{{ scope.row["gender_of_manager"] }}
									</template>
					</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="200" v-if="$check_action('/sales_manager/table','set') || $check_action('/sales_manager/view','set') || $check_action('/sales_manager/view','get')
						" >
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/sales_manager/get_list",
