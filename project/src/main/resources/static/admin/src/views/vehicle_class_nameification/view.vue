<template>
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
												<el-col v-if="$check_field('get','model_name') || $check_field('add','model_name') || $check_field('set','model_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
					...
					<div slot="label">
						...
						车型名称
							  ...
							  v-if="(form['vehicle_class_nameification_id'] && $check_field('set','model_name')) || (!form['vehicle_class_nameification_id'] && $check_field('add','model_name'))" :disabled="disabledObj['model_name_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','model_name')">
						{{form['model_name']}}
				  					</div>
														</el-form-item>
			</el-col>
				...
				<el-form-item v-if="$check_action('/vehicle_class_nameification/view','set') || $check_action('/vehicle_class_nameification/view','add')">
				...
				return ret;
					...
					async submit_after(json, func) {
						...
						let skip_Path  = path.replace('/view','/table')
								...
								title: '车型分类',
								type: '消息',
								content: this.$store.state.user.username + '-' + this.$store.state.user.nickname + '提交了一条车型分类数据',
								state: 1,
								user_id:'9999'
							}
