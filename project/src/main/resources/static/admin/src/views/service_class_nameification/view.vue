		...
		<el-row class="row_ce"> 
					...
					<div slot="label">
						...
						服务类型
							  ...
							  v-if="(form['service_class_nameification_id'] && $check_field('set','service_type')) || (!form['service_class_nameification_id'] && $check_field('add','service_type'))" :disabled="disabledObj['service_type_isDisabled']">
				  					</el-input>
					<div v-else-if="$check_field('get','service_type')">
						{{form['service_type']}}
				  					</div>
														</el-form-item>
			</el-col>
				...
				<el-form-item v-if="$check_action('/service_class_nameification/view','set') || $check_action('/service_class_nameification/view','add')">
	...
	import mixin from "@/mixins/page.js";
		...
		data() {
			return {
				...
				url_get_list: "~/api/service_class_nameification/get_list?",
				...
				return ret;
					...
					async submit_after(json, func) {
						...
						let skip_Path  = path.replace('/view','/table')
								...
								title: '服务分类',
								type: '消息',
								content: this.$store.state.user.username + '-' + this.$store.state.user.nickname + '提交了一条服务分类数据',
								state: 1,
								user_id:'9999'
							}
