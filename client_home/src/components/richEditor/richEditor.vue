    ...
    <view class="page-body">
      <view class="wrapper">
        <view class="toolbar" @tap="format" style="height: 120px; overflow-y: auto">
          ...
          ></view>
          <!-- AI接入 - 开始 -->
          <view class="iconfont icon-Ai" @tap="AIActionSheet"></view>
          <!-- AI接入 - 结束 -->
        </view>
        ...
        <!-- AI接入 - 开始 -->
        <view style="display: none" v-html="md.render(answerMD)" ref="answerRef"></view>
        <!-- AI接入 - 结束 -->
      </view>
    </view>
  </view>
...
export default {
  ...
  methods: {
      ...
      uni
        ...
        .context((nodeContext) => {
          if (nodeContext.context) {
            nodeContext.context.setContents({ html });
          }
        })
        .exec();
    },
    ...
    onInput(e) {
      ...
      this.$emit('re-change', e.detail.html);
    },
    // AI接入 - 开始
    AIActionSheet() {
      uni.showActionSheet({
        itemList: ['AI续写', 'AI优化', 'AI校对', 'AI翻译'],
        success: (res) => {
          uni.showLoading({
            title: '处理中',
          });
          ...
          $aiPost({
            messages: [
              {
                role: 'user',
                content: prompt + (this.content ? JSON.stringify(this.content) : ''),
              },
            ],
          }).then((res) => {
            uni.hideLoading();
            this.answerMD = res?.choices?.[0]?.message?.content || '分析失败，请重试';
            this.initHTML(this.answerMD);
            this.$emit('re-change', this.answerMD);
          });
        },
      });
    },
...
.page-body {
  height: calc(50vh - var(--window-top) - var(--status-bar-height));
}
...
.editor-wrapper {
  height: calc(50vh - var(--window-top) - var(--status-bar-height) - 140px);
  background: #fff;
}
