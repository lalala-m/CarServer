  ...
  <view class="page_user_address_edit" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    ...
    <view class="user_address_edit_wrapper" id="user_address_edit">
      <view class="content">
		...
		<view class="row">
			<view class="me-btn btn-save location-btn" @click="getLongitudeLatitude">定位 </view>
		</view>
      </view>
          ...
          uni.request({
            ...
            success: function (res) {
              if (res.statusCode === 200) {
                let data = res.data;
                if (data.status === '1') {
                  _this.$set(_this.form, 'address', data.regeocode.formatted_address);
                  _this.$set(_this.form, 'location_lng', JSON.stringify(lng));
                  _this.$set(_this.form, 'location_lat', JSON.stringify(lat));
                } else {
