      ...
      <view class="search-wrap">
		...
		<view class="toolbar">
		  <view
		    ...
		    @click="$navTo('/pagesA/logistics_delivery/view?')"
          ...
          <view v-for="(o, i) in list" :key="i" class="list-item">
            ...
            <view class="operate-bar">
              <view
                ...
                "
                @click="$navTo('/pagesA/logistics_delivery/view?' + field + '=' + o[field])"
              >
                详情
              </view>
              <view
...
export default {
  ...
  data() {
    return {
      // 获取数据地址
      url_get_list: '~/api/logistics_delivery/get_list?like=1',
      ...
      list: [],
      // 配送状态列表
      list_delivery_status: [{ value: '', text: '全部' },{ value: '配送中', text: '配送中' },{ value: '已完成', text: '已完成' }],
      // 签收状态列表
      list_signing_status: [{ value: '', text: '全部' },{ value: '配送中', text: '配送中' },{ value: '待签收', text: '待签收' },{ value: '已签收', text: '已签收' }],
    };
  },
  methods: {
    search_() {
    ...
    changeSigning(query, status, index,o) {
      var url = '~/api/logistics_delivery/set?' + query;
      this.list[index]['signing_status'] = status = '已签收';
      this.$post(url, { signing_status: status }, (json, status) => {
        if (json.result ) {
          this.$post("~/api/order/set?order_number=" + o.order_number, { state: '已完成' }, (json, status) => {
            if (json.result) {
              uni.redirectTo({
