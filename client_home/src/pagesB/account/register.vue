  ...
  methods: {
    ...
    submit_sub(form, table) {
	  for (let index in form) {
        ...
        if('owner_user' == table){
        let owner_user_field = ['user_id','create_by', 'owners_name', 'owners_gender', 'contact_number', 'driving_license', 'member_level', 'accumulate_points'];
				if(owner_user_field.indexOf(index) == -1){
					delete form[index];
				}
      }
          if('business_user' == table){
        let business_user_field = ['user_id','create_by', 'business_name', 'business_gender', 'mobile_phone_number', 'store_location'];
				if(business_user_field.indexOf(index) == -1){
					delete form[index];
				}
      }
          if('financial_user' == table){
        let financial_user_field = ['user_id','create_by', 'employee_work_number', 'employee_name', 'financial_gender', 'contact_number', 'two_dimensional_code'];
				if(financial_user_field.indexOf(index) == -1){
					delete form[index];
				}
      }
          if('manager_user' == table){
        let manager_user_field = ['user_id','create_by', 'employee_work_number', 'employee_name', 'employee_gender', 'contact_number', 'two_dimensional_code'];
				if(manager_user_field.indexOf(index) == -1){
					delete form[index];
				}
      }
          if('sales_manager' == table){
        let sales_manager_field = ['user_id','create_by', 'manager_name', 'gender_of_manager', 'contact_number'];
				if(sales_manager_field.indexOf(index) == -1){
					delete form[index];
				}
      }
                                                	  }
