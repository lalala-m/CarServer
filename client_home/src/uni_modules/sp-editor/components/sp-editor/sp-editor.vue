<template>
	<view class="sp-editor" :style="{ '--icon-size': iconSize, '--icon-columns': iconColumns }">
		<view class="sp-editor-toolbar" v-if="!readOnly" @tap="format">
			...
			<view v-if="toolbarList.includes('ai')" class="iconfont icon-Ai" title="AI功能" @tap="AIActionSheet"></view>
		</view>
					...
					uni.showActionSheet({
						...
						success: (actionRes) => {
							...
							$aiPost({
								...
								uni.hideLoading()
								this.answerMD = aiRes?.choices?.[0]?.message?.content || '分析失败，请重试'
								// 将AI返回的内容设置到编辑器中
								this.editorCtx.setContents({ 
									html: this.answerMD,
									success: () => {
										this.$emit('input', { html: this.answerMD, text: this.answerMD }, this.editorId)
									}
								})
							}).catch((error) => {
								uni.hideLoading()
