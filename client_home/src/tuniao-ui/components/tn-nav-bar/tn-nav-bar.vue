<template>
  <view class="tn-custom-nav-bar-class tn-custom-nav-bar" :style="[navBarStyle]">
    <view class="tn-custom-nav-bar__bar" :class="[barClass]" :style="[barStyle]">
      ...
      <view class="tn-custom-nav-bar__bar__right">
        <!-- #ifdef MP-WEIXIN || MP-ALIPAY || MP-BAIDU || MP-TOUTIAO || MP-QQ -->
        <view v-if="isScan" class="tn-custom-nav-bar__bar__action" @tap="scanCode">
          <text class="tn-icon-scan tn-custom-nav-bar__bar__action--nav-scan"></text>
        </view>
        <!-- #endif -->
...
export default {
  ...
  methods: {
    ...
    async scanCode() {
      ...
      this.isScanning = false;
      uni.showToast({ title: '当前平台暂不支持扫一扫', icon: 'none' });
      // #endif
    },
  },
};
