<template>
  <view class="page_diy_view page_service_class_nameification_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>服务分类详情</tn-nav-bar>
        ...
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','service_type') || ($check_field('add','service_type') || $check_field('set','service_type'))" label="服务类型" name="service_type">
                      <uni-easyinput type="text" v-model="form['service_type']" v-if="(form['service_class_nameification_id'] && $check_field('set','service_type')) || (!form['service_class_nameification_id'] && $check_field('add','service_type'))" :disabled="disabledObj['service_type_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','service_type')">
            {{ form['service_type'] }}          </text>
                        </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/service_class_nameification/view','set') || ($check_action('/service_class_nameification/view','add') || $check_option('/service_class_nameification/table','examine'))">
...
export default {
  ...
  data() {
    return {
      ...
      field: "service_class_nameification_id",
      url_add: "~/api/service_class_nameification/add?",
      url_set: "~/api/service_class_nameification/set?",
      url_get_obj: "~/api/service_class_nameification/get_obj?",
      url_upload: "~/api/service_class_nameification/upload?",
      service_type: null,
		          query: {
      ...
      disabledObj:{
          "service_type_isDisabled": false,
        	      ...
        	      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/service_class_nameification/upload')
      },
	    };
  },
  computed: {
  ...
  methods: {
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/service_class_nameification/upload?'),
        filePath,
        name: 'file',
    ...
    change_file(key_name) {
      ...
      uni.chooseFile({
        ...
        success: (file) => {
          ...
          uploadFilePathApi(
            '/service_class_nameification/upload',
            filePath,
            undefined,
            {
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/service_class_nameification/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
    ...
    is_view() {
      ...
      if (!bl) {
        bl = this.$check_action('/service_class_nameification/table', 'add');
        console.log(bl ? '你有表格添加权限视作有添加权限' : '你没有表格添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/service_class_nameification/table', 'set');
        console.log(bl ? '你有表格添加权限视作有修改权限' : '你没有表格修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/service_class_nameification/view', 'add');
        console.log(bl ? '你有视图添加权限视作有添加权限' : '你没有视图添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/service_class_nameification/view', 'set');
        console.log(bl ? '你有视图修改权限视作有修改权限' : '你没有视图修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/service_class_nameification/view', 'get');
