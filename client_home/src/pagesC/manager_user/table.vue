<template>
  <view class="page_diy_table page_manager_user_table" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar backUrl="/pages/user/index">经理用户列表</tn-nav-bar>
    <view class="page-list" id="page_diy_table">
      ...
      <view class="search-wrap">
		...
		<view class="toolbar">
				  <view width="100%" class="me-btn btn-add" @click="$navTo('/pagesC/manager_user/view?')" v-if="$check_action('/manager_user/table', 'add') || $check_action('/manager_user/view', 'add')">添加</view>
		        		</view>
      </view>
      <!-- 列表 -->
      <view class="list-wrap-row">
        <view v-for="(o, i) in list" :key="i" class="list-item">
          ...
          <view class="item-row " v-if="1 && $check_field('get','employee_gender')">
            <view class="label" v-if="true">
              <span>员工性别</span>
            </view>
              <view class="value">
          ...
          <view class="operate-bar">
            <view width="100%" class="me-btn btn-delete" @click="delInfo(i)"
                  v-if="$check_action('/manager_user/table', 'del') || $check_action('/manager_user/view', 'del')">
              删除
            </view>
            <view width="100%" class="me-btn btn-info" @click="$navTo('/pagesC/manager_user/view?' + field + '=' + o[field])"
...
export default {
  ...
  data() {
    return {
      // 获取数据地址
      url_get_list: '~/api/manager_user/get_list?like=1',
