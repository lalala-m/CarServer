<template>
  <view class="page_diy_view page_brand_class_nameification_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>品牌分类详情</tn-nav-bar>
      ...
      <uni-forms :modelValue="form" v-if="is_view()" labelWidth="70px">
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','brand_name') || ($check_field('add','brand_name') || $check_field('set','brand_name'))" label="品牌名称" name="brand_name">
                      <uni-easyinput type="text" v-model="form['brand_name']" v-if="(form['brand_class_nameification_id'] && $check_field('set','brand_name')) || (!form['brand_class_nameification_id'] && $check_field('add','brand_name'))" :disabled="disabledObj['brand_name_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','brand_name')">
            {{ form['brand_name'] }}          </text>
                        </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/brand_class_nameification/view','set') || ($check_action('/brand_class_nameification/view','add') || $check_option('/brand_class_nameification/table','examine'))">
...
export default {
  ...
  data() {
    return {
      ...
      field: "brand_class_nameification_id",
      url_add: "~/api/brand_class_nameification/add?",
      url_set: "~/api/brand_class_nameification/set?",
      url_get_obj: "~/api/brand_class_nameification/get_obj?",
      url_upload: "~/api/brand_class_nameification/upload?",
      brand_name: null,
		          query: {
        "brand_class_nameification_id": 0,
      },
      form: {
          "brand_name": null, // 品牌名称
          "brand_class_nameification_id": 0, // ID
      },
      disabledObj:{
          "brand_name_isDisabled": false,
        },
        	      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/brand_class_nameification/upload')
      },
	    };
  },
  computed: {
  ...
  methods: {
            ...
            const navigate = () => {
				      ...
				      // #endif
						...
						if(!this.form.brand_class_nameification_id){
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/brand_class_nameification/upload?'),
        filePath,
        name: 'file',
      ...
      close_(type) {
      if (type == 'brand_name') this['brand_name'] = this.form['brand_name'] = "";
    },
    submit_() {
        if (this['brand_name'] !== null) this.form['brand_name'] = this['brand_name']
    ...
    change_file(key_name) {
      ...
      uni.chooseFile({
        ...
        success: (file) => {
          ...
          uploadFilePathApi(
            '/brand_class_nameification/upload',
            filePath,
            undefined,
            {
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/brand_class_nameification/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
    ...
    is_view() {
      ...
      if (!bl) {
        bl = this.$check_action('/brand_class_nameification/table', 'add');
        console.log(bl ? '你有表格添加权限视作有添加权限' : '你没有表格添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/brand_class_nameification/table', 'set');
        console.log(bl ? '你有表格添加权限视作有修改权限' : '你没有表格修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/brand_class_nameification/view', 'add');
        console.log(bl ? '你有视图添加权限视作有添加权限' : '你没有视图添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/brand_class_nameification/view', 'set');
        console.log(bl ? '你有视图修改权限视作有修改权限' : '你没有视图修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/brand_class_nameification/view', 'get');
