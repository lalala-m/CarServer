<template>
  <view class="page_diy_view page_vehicle_class_nameification_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>车型分类详情</tn-nav-bar>
      ...
      <uni-forms :modelValue="form" v-if="is_view()" labelWidth="70px">
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','model_name') || ($check_field('add','model_name') || $check_field('set','model_name'))" label="车型名称" name="model_name">
                      <uni-easyinput type="text" v-model="form['model_name']" v-if="(form['vehicle_class_nameification_id'] && $check_field('set','model_name')) || (!form['vehicle_class_nameification_id'] && $check_field('add','model_name'))" :disabled="disabledObj['model_name_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','model_name')">
            {{ form['model_name'] }}          </text>
                        </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/vehicle_class_nameification/view','set') || ($check_action('/vehicle_class_nameification/view','add') || $check_option('/vehicle_class_nameification/table','examine'))">
...
export default {
  ...
  data() {
    return {
      ...
      field: "vehicle_class_nameification_id",
      url_add: "~/api/vehicle_class_nameification/add?",
      url_set: "~/api/vehicle_class_nameification/set?",
      url_get_obj: "~/api/vehicle_class_nameification/get_obj?",
      url_upload: "~/api/vehicle_class_nameification/upload?",
      model_name: null,
		          query: {
        "vehicle_class_nameification_id": 0,
      },
      oldForm: {},
      form: {
          "model_name": null, // 车型名称
          "vehicle_class_nameification_id": 0, // ID
        "create_by": 0, // 创建人
      },
      disabledObj:{
          "model_name_isDisabled": false,
        	      ...
        	      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/vehicle_class_nameification/upload')
      },
	    };
  },
  computed: {
  ...
  methods: {
            ...
            const navigate = () => {
				      ...
				      // #endif
    						...
    						if(!this.form.vehicle_class_nameification_id){
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/vehicle_class_nameification/upload?'),
        filePath,
        name: 'file',
      ...
      close_(type) {
      if (type == 'model_name') this['model_name'] = this.form['model_name'] = "";
    },
    submit_() {
        if (this['model_name'] !== null) this.form['model_name'] = this['model_name']
    ...
    change_file(key_name) {
      ...
      uni.chooseFile({
        ...
        success: (file) => {
          ...
          uploadFilePathApi(
            '/vehicle_class_nameification/upload',
            filePath,
            undefined,
            {
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/vehicle_class_nameification/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
    ...
    is_view() {
      ...
      if (!bl) {
        bl = this.$check_action('/vehicle_class_nameification/table', 'add');
        console.log(bl ? '你有表格添加权限视作有添加权限' : '你没有表格添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/vehicle_class_nameification/table', 'set');
        console.log(bl ? '你有表格添加权限视作有修改权限' : '你没有表格修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/vehicle_class_nameification/view', 'add');
        console.log(bl ? '你有视图添加权限视作有添加权限' : '你没有视图添加权限');
      }
      if (!bl) {
        bl = this.$check_action('/vehicle_class_nameification/view', 'set');
        console.log(bl ? '你有视图修改权限视作有修改权限' : '你没有视图修改权限');
      }
      if (!bl) {
        bl = this.$check_action('/vehicle_class_nameification/view', 'get');
