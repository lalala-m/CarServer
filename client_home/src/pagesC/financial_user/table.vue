<template>
  <view class="page_diy_table page_financial_user_table" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar backUrl="/pages/user/index">财务用户列表</tn-nav-bar>
    <view class="page-list" id="page_diy_table">
      ...
      <view class="search-wrap">
		...
		<view class="toolbar">
				  <view width="100%" class="me-btn btn-add" @click="$navTo('/pagesC/financial_user/view?')" v-if="$check_action('/financial_user/table', 'add') || $check_action('/financial_user/view', 'add')">添加</view>
		        		</view>
      </view>
      <!-- 列表 -->
      <view class="list-wrap-row">
        <view v-for="(o, i) in list" :key="i" class="list-item">
          <view class="item-row " v-if="0 && $check_field('get','employee_work_number')">
            <view class="label" v-if="true">
              <span>员工工号</span>
            </view>
              <view class="value">
                              <span>{{ o['employee_work_number'] }}</span>
                          </view>
            </view>
          <view class="item-row " v-if="1 && $check_field('get','employee_name')">
            <view class="label" v-if="true">
              <span>员工姓名</span>
            </view>
              <view class="value">
          ...
          <view class="item-row " v-if="1 && $check_field('get','financial_gender')">
            <view class="label" v-if="true">
              <span>员工性别</span>
            </view>
              <view class="value">
                              <span>{{ o['financial_gender'] }}</span>
                          </view>
            </view>
          ...
          <view class="item-row diy-image" v-if="1 && $check_field('get','two_dimensional_code')">
            <view class="label" v-if=" false">
              <span>二维码</span>
            </view>
              <view class="value diy_img">
              <image :src="$fullImgUrl(o['two_dimensional_code'])" width="100%" height="100" />
            </view>
            </view>
          <view class="content">
          </view>
          <view class="operate-bar">
            <view width="100%" class="me-btn btn-delete" @click="delInfo(i)"
                  v-if="$check_action('/financial_user/table', 'del') || $check_action('/financial_user/view', 'del')">
              删除
            </view>
            <view width="100%" class="me-btn btn-info" @click="$navTo('/pagesC/financial_user/view?' + field + '=' + o[field])"
...
export default {
  ...
  data() {
    return {
      // 获取数据地址
      url_get_list: '~/api/financial_user/get_list?like=1',
