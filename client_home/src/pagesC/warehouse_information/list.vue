<template>
  <view :style="{ paddingTop: vuex_custom_bar_height + 'px' }" class="page_diy_list page_warehouse_information_list container">
	<view class="content">
	  <tn-nav-bar :isBack="isBack">仓库信息列表</tn-nav-bar>
	  <view class="page-list" id="page_diy_list">
	        <!-- 筛选模块(开始) -->
																													        <view class="search-wrap">
	  	    	      	          <Search v-model="query.warehouse_name" placeholder="搜索仓库名称" @search="search_" @cancel="search_cancel('warehouse_name')" @input="(val) => inputValue(val, 'warehouse_name')" />
...
export default {
  ...
  data() {
    return {
      ...
      url_get_list: '~/api/warehouse_information/get_list?like=1',
                    // 默认状态选项列表
      list_default_state: [{value:"全部",text:"全部"}],
	  placeholder_default_state: "请选择默认状态",
	...
	computed: {
		...
		hasImageItems() {
			return this.showList.some(item => {
																								if (item['warehouse_picture'] && item['warehouse_picture'] !== '') {
							return true;
						}
																																								return false;
			});
  ...
  methods: {
	toDetails(o) {
						this.$navTo('/pagesC/warehouse_information/details?warehouse_information_id=' + o['warehouse_information_id'])
			},
    ...
    get_list_after(json, func) {
      ...
      if (func) {
        func(json);
					                                        ...
					                                        // 改变分类标签
        change_default_state(val) {
          if (val === "全部" || val === "") {
            this.query.default_state = "";
          }else{
            this.query.default_state = val
          }
          this.search_();
        },
                /**
