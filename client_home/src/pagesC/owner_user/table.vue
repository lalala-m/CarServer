<template>
  <view class="page_diy_table page_owner_user_table" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar backUrl="/pages/user/index">车主用户列表</tn-nav-bar>
    <view class="page-list" id="page_diy_table">
      ...
      <view class="search-wrap">
		...
		<view class="toolbar">
				  <view width="100%" class="me-btn btn-add" @click="$navTo('/pagesC/owner_user/view?')" v-if="$check_action('/owner_user/table', 'add') || $check_action('/owner_user/view', 'add')">添加</view>
		        		</view>
      </view>
      <!-- 列表 -->
      <view class="list-wrap-row">
        <view v-for="(o, i) in list" :key="i" class="list-item">
          ...
          <view class="item-row " v-if="1 && $check_field('get','owners_gender')">
            <view class="label" v-if="true">
              <span>车主性别</span>
            </view>
              <view class="value">
                              <span>{{ o['owners_gender'] }}</span>
                          </view>
            </view>
          ...
          <view class="item-row diy-image" v-if="1 && $check_field('get','driving_license')">
            <view class="label" v-if=" false">
              <span>驾驶证件</span>
            </view>
              <view class="value diy_img">
              <image :src="$fullImgUrl(o['driving_license'])" width="100%" height="100" />
            </view>
            </view>
          <view class="item-row " v-if="0 && $check_field('get','member_level')">
            <view class="label" v-if="true">
              <span>会员等级</span>
            </view>
              <view class="value">
                              <span>{{ o['member_level'] }}</span>
                          </view>
            </view>
          <view class="item-row " v-if="0 && $check_field('get','accumulate_points')">
            <view class="label" v-if="true">
              <span>积攒积分</span>
            </view>
              <view class="value">
                              <span>{{ o['accumulate_points'] }}分</span>
                          </view>
            </view>
          <view class="content">
          </view>
          <view class="operate-bar">
            <view width="100%" class="me-btn btn-delete" @click="delInfo(i)"
                  v-if="$check_action('/owner_user/table', 'del') || $check_action('/owner_user/view', 'del')">
              删除
            </view>
            <view width="100%" class="me-btn btn-info" @click="$navTo('/pagesC/owner_user/view?' + field + '=' + o[field])"
...
export default {
  ...
  data() {
    return {
      // 获取数据地址
      url_get_list: '~/api/owner_user/get_list?like=1',
      ...
      query: {
        ...
        create_time: '',
						      ...
						      // 数据
      list: [],
                                                          	  accumulate_points_input_timer: null,
        };
  },
  methods: {
                        								accumulate_points_input() {
			this.accumulate_points_input_timer && clearTimeout(this.accumulate_points_input_timer);
			this.accumulate_points_input_timer = setTimeout(() => {
				if((this.query.accumulate_points_min && this.query.accumulate_points_max) || (!this.query.accumulate_points_min && !this.query.accumulate_points_max)) {
					this.search_();
				}
			}, 300);
		},
