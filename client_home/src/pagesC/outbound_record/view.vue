<template>
  <view class="page_diy_view page_outbound_record_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>出库记录详情</tn-nav-bar>
        ...
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','consumables_code') || ($check_field('add','consumables_code') || $check_field('set','consumables_code'))" label="耗材编码" name="consumables_code">
                      <uni-easyinput type="text" v-model="form['consumables_code']" v-if="(form['outbound_record_id'] && $check_field('set','consumables_code')) || (!form['outbound_record_id'] && $check_field('add','consumables_code'))" :disabled="disabledObj['consumables_code_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_code')">
            {{ form['consumables_code'] }}          </text>
              ...
              <uni-forms-item class="" v-if="$check_field('get','consumables_name') || ($check_field('add','consumables_name') || $check_field('set','consumables_name'))" label="耗材名称" name="consumables_name">
                      <uni-easyinput type="text" v-model="form['consumables_name']" v-if="(form['outbound_record_id'] && $check_field('set','consumables_name')) || (!form['outbound_record_id'] && $check_field('add','consumables_name'))" :disabled="disabledObj['consumables_name_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_name')">
            {{ form['consumables_name'] }}          </text>
              ...
              <uni-forms-item class="" v-if="$check_field('get','consumables_specifications') || ($check_field('add','consumables_specifications') || $check_field('set','consumables_specifications'))" label="耗材规格" name="consumables_specifications">
                      <uni-easyinput type="text" v-model="form['consumables_specifications']" v-if="(form['outbound_record_id'] && $check_field('set','consumables_specifications')) || (!form['outbound_record_id'] && $check_field('add','consumables_specifications'))" :disabled="disabledObj['consumables_specifications_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_specifications')">
            {{ form['consumables_specifications'] }}          </text>
              ...
              v-if="(form['outbound_record_id'] && $check_field('set', 'financial_user')) || (!form['outbound_record_id'] && $check_field('add', 'financial_user'))"
			   style="width: 100%;"
          ></uni-data-select>
          <uni-data-select
              v-model="form['financial_user']"
          ...
          ></uni-data-select>
              ...
              <uni-forms-item class="" v-if="$check_field('get','outbound_time') || ($check_field('add','outbound_time') || $check_field('set','outbound_time'))" label="出库时间" name="outbound_time">
              <uni-datetime-picker v-if="(form['outbound_record_id'] && $check_field('set', 'outbound_time')) || (!form['outbound_record_id'] && $check_field('add', 'outbound_time'))" @change="changeLog($event, 'outbound_time')" v-model="form['outbound_time']" type="datetime" :disabled="disabledObj['outbound_time_isDisabled']"/>
          <!-- 仅查看 -->
          <text v-else-if="$check_field('get', 'outbound_time')">
            {{ form['outbound_time'] }}
          </text>
                </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','outbound_quantity') || ($check_field('add','outbound_quantity') || $check_field('set','outbound_quantity'))" label="出库数量" name="outbound_quantity">
                      <uni-easyinput type="text" v-model="form['outbound_quantity']" v-if="(form['outbound_record_id'] && $check_field('set','outbound_quantity')) || (!form['outbound_record_id'] && $check_field('add','outbound_quantity'))" :disabled="disabledObj['outbound_quantity_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','outbound_quantity')">
            {{ form['outbound_quantity'] }}          </text>
                        </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','outbound_note') || ($check_field('add','outbound_note') || $check_field('set','outbound_note'))" label="出库备注" name="outbound_note">
              <uni-easyinput type="textarea" v-model="form['outbound_note']" v-if="(form['outbound_record_id'] && $check_field('set', 'outbound_note')) || (!form['outbound_record_id'] && $check_field('add', 'outbound_note'))" :disabled="disabledObj['outbound_note_isDisabled']"/>
          <!-- 仅查看 -->
          <text v-else-if="$check_field('get', 'outbound_note')">
            {{ form['outbound_note'] }}
          </text>
                </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/outbound_record/view','set') || ($check_action('/outbound_record/view','add') || $check_option('/outbound_record/table','examine'))">
...
export default {
    ...
    return {
      ...
      form: {
          "consumables_code":  '', // 耗材编码
            ...
            "outbound_time": this.$toTime(new Date().getTime(), "yyyy-MM-dd hh:mm:ss"),
      ...
      group_user_financial_user: "",
                                    							      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/outbound_record/upload')
      },
							    };
  },
  computed: {
  ...
  methods: {
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/outbound_record/upload?'),
        filePath,
        name: 'file',
      ...
      if (type == 'outbound_time') this['outbound_time'] = this.form['outbound_time'] = "";
      if (type == 'outbound_quantity') this['outbound_quantity'] = this.form['outbound_quantity'] = "";
      if (type == 'outbound_note') this['outbound_note'] = this.form['outbound_note'] = "";
    },
    submit_() {
        if (this['consumables_code'] !== null) this.form['consumables_code'] = this['consumables_code']
          ...
          if (this['outbound_time'] !== null) this.form['outbound_time'] = this['outbound_time']
          if (this['outbound_quantity'] !== null) this.form['outbound_quantity'] = this['outbound_quantity']
          if (this['outbound_note'] !== null) this.form['outbound_note'] = this['outbound_note']
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/outbound_record/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
