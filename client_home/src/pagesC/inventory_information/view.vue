<template>
  <view class="page_diy_view page_inventory_information_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>库存信息详情</tn-nav-bar>
        ...
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','consumables_code') || ($check_field('add','consumables_code') || $check_field('set','consumables_code'))" label="耗材编码" name="consumables_code">
                      <uni-easyinput type="text" v-model="form['consumables_code']" v-if="(form['inventory_information_id'] && $check_field('set','consumables_code')) || (!form['inventory_information_id'] && $check_field('add','consumables_code'))" :disabled="disabledObj['consumables_code_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_code')">
            {{ form['consumables_code'] }}          </text>
              ...
              <uni-forms-item class="" v-if="$check_field('get','consumables_name') || ($check_field('add','consumables_name') || $check_field('set','consumables_name'))" label="耗材名称" name="consumables_name">
                      <uni-easyinput type="text" v-model="form['consumables_name']" v-if="(form['inventory_information_id'] && $check_field('set','consumables_name')) || (!form['inventory_information_id'] && $check_field('add','consumables_name'))" :disabled="disabledObj['consumables_name_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_name')">
            {{ form['consumables_name'] }}          </text>
                        </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','consumables_inventory') || ($check_field('add','consumables_inventory') || $check_field('set','consumables_inventory'))" label="耗材库存" name="consumables_inventory">
                      <uni-easyinput type="text" v-model="form['consumables_inventory']" v-if="(form['inventory_information_id'] && $check_field('set','consumables_inventory')) || (!form['inventory_information_id'] && $check_field('add','consumables_inventory'))" :disabled="disabledObj['consumables_inventory_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_inventory')">
            {{ form['consumables_inventory'] }}          </text>
                        </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','consumables_specifications') || ($check_field('add','consumables_specifications') || $check_field('set','consumables_specifications'))" label="耗材规格" name="consumables_specifications">
                      <uni-easyinput type="text" v-model="form['consumables_specifications']" v-if="(form['inventory_information_id'] && $check_field('set','consumables_specifications')) || (!form['inventory_information_id'] && $check_field('add','consumables_specifications'))" :disabled="disabledObj['consumables_specifications_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','consumables_specifications')">
            {{ form['consumables_specifications'] }}          </text>
                        </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','introduction_of_consumables') || ($check_field('add','introduction_of_consumables') || $check_field('set','introduction_of_consumables'))" label="耗材简介" name="introduction_of_consumables">
              <uni-easyinput type="textarea" v-model="form['introduction_of_consumables']" v-if="(form['inventory_information_id'] && $check_field('set', 'introduction_of_consumables')) || (!form['inventory_information_id'] && $check_field('add', 'introduction_of_consumables'))" :disabled="disabledObj['introduction_of_consumables_isDisabled']"/>
          <!-- 仅查看 -->
          <text v-else-if="$check_field('get', 'introduction_of_consumables')">
            {{ form['introduction_of_consumables'] }}
          </text>
                </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/inventory_information/view','set') || ($check_action('/inventory_information/view','add') || $check_option('/inventory_information/table','examine'))">
...
export default {
  ...
  data() {
    return {
      ...
      consumables_code: null,
	      ...
	      consumables_inventory: null,
	      consumables_specifications: null,
	      introduction_of_consumables: null,
						                          query: {
      ...
      disabledObj:{
          "consumables_code_isDisabled": false,
            ...
            "introduction_of_consumables_isDisabled": false,
                                        					      ...
                                        					      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/inventory_information/upload')
      },
					    };
  },
  computed: {
  ...
  methods: {
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: this.$fullUrl('~/api/inventory_information/upload?'),
        filePath,
        name: 'file',
      ...
      if (type == 'consumables_inventory') this['consumables_inventory'] = this.form['consumables_inventory'] = "";
      if (type == 'consumables_specifications') this['consumables_specifications'] = this.form['consumables_specifications'] = "";
      if (type == 'introduction_of_consumables') this['introduction_of_consumables'] = this.form['introduction_of_consumables'] = "";
    },
    submit_() {
        if (this['consumables_code'] !== null) this.form['consumables_code'] = this['consumables_code']
          ...
          if (this['consumables_inventory'] !== null) this.form['consumables_inventory'] = this['consumables_inventory']
          if (this['consumables_specifications'] !== null) this.form['consumables_specifications'] = this['consumables_specifications']
          if (this['introduction_of_consumables'] !== null) this.form['introduction_of_consumables'] = this['introduction_of_consumables']
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/inventory_information/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
