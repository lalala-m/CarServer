<template>
  <view class="page_diy_view page_test_drive_record_view" :style="{ paddingTop: vuex_custom_bar_height + 'px' }">
    <tn-nav-bar>试驾记录详情</tn-nav-bar>
      ...
      <uni-forms :modelValue="form" v-if="is_view()" labelWidth="70px">
        <template v-if="form['extra']">
          <uni-forms-item class="" v-for="(value, key) in form['extra']" :key="key" :label="key" :name="key">
            ...
            <uni-forms-item class="" v-if="$check_field('get','test_drive_code') || ($check_field('add','test_drive_code') || $check_field('set','test_drive_code'))" label="试驾编码" name="test_drive_code">
                      <uni-easyinput type="text" v-model="form['test_drive_code']" v-if="(form['test_drive_record_id'] && $check_field('set','test_drive_code')) || (!form['test_drive_record_id'] && $check_field('add','test_drive_code'))"  :disabled="true">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','test_drive_code')">
            {{ form['test_drive_code'] }}          </text>
                        </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','car_coding') || ($check_field('add','car_coding') || $check_field('set','car_coding'))" label="汽车编码" name="car_coding">
                      <uni-easyinput type="text" v-model="form['car_coding']" v-if="(form['test_drive_record_id'] && $check_field('set','car_coding')) || (!form['test_drive_record_id'] && $check_field('add','car_coding'))" :disabled="disabledObj['car_coding_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','car_coding')">
            {{ form['car_coding'] }}          </text>
              ...
              <uni-forms-item class="" v-if="$check_field('get','car_name') || ($check_field('add','car_name') || $check_field('set','car_name'))" label="汽车名称" name="car_name">
                      <uni-easyinput type="text" v-model="form['car_name']" v-if="(form['test_drive_record_id'] && $check_field('set','car_name')) || (!form['test_drive_record_id'] && $check_field('add','car_name'))" :disabled="disabledObj['car_name_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','car_name')">
            {{ form['car_name'] }}          </text>
              ...
              <uni-forms-item class="" v-if="$check_field('get','car_models') || ($check_field('add','car_models') || $check_field('set','car_models'))" label="汽车车型" name="car_models">
                      <uni-easyinput type="text" v-model="form['car_models']" v-if="(form['test_drive_record_id'] && $check_field('set','car_models')) || (!form['test_drive_record_id'] && $check_field('add','car_models'))" :disabled="disabledObj['car_models_isDisabled']">
                      </uni-easyinput>
		  <!-- 仅查看 -->
          <text v-else-if="$check_field('get','car_models')">
            {{ form['car_models'] }}          </text>
              ...
              v-if="(form['test_drive_record_id'] && $check_field('set', 'business_user')) || (!form['test_drive_record_id'] && $check_field('add', 'business_user'))"
			   style="width: 100%;"
          ></uni-data-select>
          <uni-data-select
              v-model="form['business_user']"
          ...
          ></uni-data-select>
              ...
              v-if="(form['test_drive_record_id'] && $check_field('set', 'owner_user')) || (!form['test_drive_record_id'] && $check_field('add', 'owner_user'))"
			   style="width: 100%;"
          ></uni-data-select>
          <uni-data-select
              v-model="form['owner_user']"
          ...
          ></uni-data-select>
              ...
              <uni-forms-item class="" v-if="$check_field('get','test_drive_time') || ($check_field('add','test_drive_time') || $check_field('set','test_drive_time'))" label="试驾时间" name="test_drive_time">
              <uni-datetime-picker v-if="(form['test_drive_record_id'] && $check_field('set', 'test_drive_time')) || (!form['test_drive_record_id'] && $check_field('add', 'test_drive_time'))" @change="changeLog($event, 'test_drive_time')" v-model="form['test_drive_time']" type="datetime" :disabled="disabledObj['test_drive_time_isDisabled']"/>
          <!-- 仅查看 -->
          <text v-else-if="$check_field('get', 'test_drive_time')">
            {{ form['test_drive_time'] }}
          </text>
                </uni-forms-item>
              <uni-forms-item class="" v-if="$check_field('get','test_drive_remarks') || ($check_field('add','test_drive_remarks') || $check_field('set','test_drive_remarks'))" label="试驾备注" name="test_drive_remarks">
              <uni-easyinput type="textarea" v-model="form['test_drive_remarks']" v-if="(form['test_drive_record_id'] && $check_field('set', 'test_drive_remarks')) || (!form['test_drive_record_id'] && $check_field('add', 'test_drive_remarks'))" :disabled="disabledObj['test_drive_remarks_isDisabled']"/>
          <!-- 仅查看 -->
          <text v-else-if="$check_field('get', 'test_drive_remarks')">
            {{ form['test_drive_remarks'] }}
          </text>
                </uni-forms-item>
        </uni-forms>
      <view class="form-footer" v-if="$check_action('/test_drive_record/view','set') || ($check_action('/test_drive_record/view','add') || $check_option('/test_drive_record/table','examine'))">
...
export default {
  ...
  data() {
    return {
      ...
      group_user_owner_user: "",
                            								      uploadKey: '',
      uploadOptions: {
        url: this.$fullImgUrl('~/api/test_drive_record/upload')
      },
								    };
  },
  computed: {
  ...
  methods: {
    ...
    successChoose(filePath, type) {
        ...
        success: (uploadFileRes) => {
          if (uploadFileRes.data.error) {
            ...
            this[type] = JSON.parse(uploadFileRes.data).result.url;
                    ...
                    close_(type) {
      if (type == 'test_drive_code') this['test_drive_code'] = this.form['test_drive_code'] = "";
      ...
      if (type == 'test_drive_time') this['test_drive_time'] = this.form['test_drive_time'] = "";
      if (type == 'test_drive_remarks') this['test_drive_remarks'] = this.form['test_drive_remarks'] = "";
    },
    submit_() {
        if (this['test_drive_code'] !== null) this.form['test_drive_code'] = this['test_drive_code']
    ...
    change_img(key_name) {
      ...
      uni.chooseImage({
        ...
        success: (file) => {
          const tempFilePaths = file.tempFilePaths;
          const filePath = tempFilePaths[0];
          uploadFilePathApi('/test_drive_record/upload',filePath,undefined,undefined,
            (task) => {
              task.onProgressUpdate((res) => {
                this.percent = res.progress;
                console.log('上传进度' + res.progress);
